...
tipo_di_ambiente = "NaCl" #NaCl,BULK sono le opzioni, indica il tipo di ambiente in cui è inserita l'acqua
#OPZIONI SECONDARIE
    ...
    for x in range(len(files_pdb)):
        ...
        if funzione_della_distanza_onoff == "ON" and calcolo_angoli_onoff == "ON":
            ...
            for distanza_istogrammi in np.arange(6.5, 25.5, 0.5):
                # ISOLE
                plt.hist((angoli_isole_distanza_database[counter_istogrammi_angoli]), bins=30, range=(-1, 1))
                plt.gca().yaxis.set_major_formatter(PercentFormatter(1))
                plt.xticks(np.arange(-1, 1, 0.1), rotation=30)
                plt.xlabel('Angolo(°)')
                plt.ylabel('Frequenza')
                plt.savefig(path_output_istogrammi_angoli + "Isole\\" + str(distanza_istogrammi-6.0) + ".png")
                plt.close()
                # LAYERS
                plt.hist((angoli_layers_distanza_database[counter_istogrammi_angoli]), bins=30, range=(-1, 1))
                plt.gca().yaxis.set_major_formatter(PercentFormatter(1))
                plt.xticks(np.arange(-1, 1, 0.1), rotation=30)
                plt.xlabel('Angolo(°)')
                plt.ylabel('Frequenza')
                ...
                plt.close()
                # RUMORE
                plt.hist((angoli_rumore_distanza_database[counter_istogrammi_angoli]), bins=30, range=(-1, 1))
                plt.gca().yaxis.set_major_formatter(PercentFormatter(1))
                plt.xticks(np.arange(-1, 1, 0.1), rotation=30)
                plt.xlabel('Angolo(°)')
                plt.ylabel('Frequenza')
        ...
        if funzione_della_distanza_onoff == "OFF" and calcolo_angoli_onoff == "ON":
            ...
            # ISOLE
            plt.hist((angoli_isole_distanza_database), bins=30, range=(-1, 1), weights= np.ones(len(angoli_isole_distanza_database)) / len(angoli_isole_distanza_database))
            plt.gca().yaxis.set_major_formatter(PercentFormatter(1))
            plt.xticks(np.arange(-1, 1, 0.1), rotation=30)
            plt.xlabel('Angolo(°)')
            plt.ylabel('Frequenza')
            ...
            plt.close()
            # LAYERS
            plt.hist((angoli_layers_distanza_database), bins=30, range=(-1, 1), weights= np.ones(len(angoli_layers_distanza_database)) / len(angoli_layers_distanza_database))
            plt.gca().yaxis.set_major_formatter(PercentFormatter(1))
            plt.xticks(np.arange(-1, 1, 0.1), rotation=30)
            plt.xlabel('Angolo(°)')
            plt.ylabel('Frequenza')
            ...
            plt.close()
            # RUMORE
            plt.hist((angoli_rumore_distanza_database), bins=18, range=(-1, 1), weights= np.ones(len(angoli_rumore_distanza_database)) / len(angoli_rumore_distanza_database))
            plt.gca().yaxis.set_major_formatter(PercentFormatter(1))
            plt.xticks(np.arange(-1, 1, 0.1), rotation=30)
            plt.xlabel('Angolo(°)')
            plt.ylabel('Frequenza')
