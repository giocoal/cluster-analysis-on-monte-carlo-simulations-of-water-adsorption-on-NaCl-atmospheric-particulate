...
funzione_della_distanza_onoff = "OFF" #gli angoli vengono calcolati in funzione della distanza dalla superficie
tipo_di_ambiente = "BULK" #Indica il tipo di ambiente in cui è inserita l'acqua, Parametri possibili: NaCl - BULK
...
outofbox_onoff = "OFF" #reinserisce molecole erroneamente fuori dalla scatola sulla base della periodicità
#PATH INPUT
cartella_e_nome_output = "Replica2"
...
if tipo_di_ambiente == "BULK":
    numero_di_frame = 2 #numero di frame da analizzare per ogni file
if tipo_di_ambiente == "NaCl":
...
# CARICAMENTO FILES
    ...
    for x in range(len(files_pdb)):
        ...
        if funzione_della_distanza_onoff == "ON":
            ...
            plt.ylabel(
                'Angolo medio tra il vettore momento di dipolo normalizzato acqua e vettore normale alla superficie di NaCl(°)',
            ...
            # Vado a plottare isole,layers,rumore in un unico grafico, per ognuno di essi va inserita prima la X(distanza)
            # Gli ultimi tre parametri sono grafici, l'ultimo setta su non il "riempimento" dei simboli, che altrimenti sarebbero pieni
            plt.plot(distanza_asseX, angoli_isole_medio_std, 'bo', distanza_asseX, angoli_layers_medio_std, 'rs',
                     distanza_asseX,
                     angoli_rumore_medio_std, 'g+', linewidth=1, markersize=8, mfc='none')
            # L'immagine avrà una griglia tratteggiata
            plt.grid(True, linestyle='--')
            # Setto i "divisori" in ogni asse,  con i relativi step ed eventuale rotazione(utile se ci sono sovrapposizioni)
            plt.xticks(np.arange(0.5, 19.5, 1.0), rotation=30)
        ...
        if funzione_della_distanza_onoff == "ON" and calcolo_angoli_onoff == "ON":
            ...
            for distanza_istogrammi in np.arange(6.5, 25.5, 0.5):
                if unità_di_misura_angoli == "GRADI":
                    # ISOLE
                    plt.hist((angoli_isole_distanza_database[counter_istogrammi_angoli]), bins=18, range=(0, 180),
                             weights=np.ones(len(angoli_isole_distanza_database[counter_istogrammi_angoli])) / len(
                                 angoli_isole_distanza_database[counter_istogrammi_angoli]))
                    plt.gca().yaxis.set_major_formatter(PercentFormatter(1))
                    plt.xticks(np.arange(0, 180, 5), rotation=30)
                    plt.xlabel('θ(°)')
                    plt.ylabel('Frequenza')
                    plt.savefig(path_output_istogrammi_angoli + "Isole\\" + str(distanza_istogrammi - 6.0) + ".png")
                    plt.close()
                    # LAYERS
                    plt.hist((angoli_layers_distanza_database[counter_istogrammi_angoli]), bins=18, range=(0, 180),
                    ...
                    plt.close()
                    # RUMORE
                    plt.hist((angoli_rumore_distanza_database[counter_istogrammi_angoli]), bins=18, range=(0, 180),
                ...
                if unità_di_misura_angoli == "COSENO":
                    # ISOLE
                    plt.hist((angoli_isole_distanza_database[counter_istogrammi_angoli]), bins=20, range=(-1.2,1.2),
                             weights=np.ones(len(angoli_isole_distanza_database[counter_istogrammi_angoli])) / len(
                                 angoli_isole_distanza_database[counter_istogrammi_angoli]))
                    plt.gca().yaxis.set_major_formatter(PercentFormatter(1))
                    plt.xticks(np.arange(-1, 1, 0.2), rotation=30)
                    plt.xlabel('θ(°)')
                    plt.ylabel('Frequenza')
                    plt.savefig(path_output_istogrammi_angoli + "Isole\\" + str(distanza_istogrammi - 6.0) + ".png")
                    plt.close()
        ...
        if funzione_della_distanza_onoff == "OFF" and calcolo_angoli_onoff == "ON":
            if unità_di_misura_angoli == "GRADI":
                ...
                plt.close()
                # LAYERS
                plt.hist((angoli_layers_distanza_database), bins=30, range=(0, 190), weights= np.ones(len(angoli_layers_distanza_database)) / len(angoli_layers_distanza_database))
                plt.gca().yaxis.set_major_formatter(PercentFormatter(1))
                plt.xticks(np.arange(0, 190, 10), rotation=30)
                plt.xlabel('θ(°)')
                plt.ylabel('Frequenza')
