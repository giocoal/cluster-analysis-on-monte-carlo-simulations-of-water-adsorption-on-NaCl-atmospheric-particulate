...
"IMPOSTAZIONI"
#OPZIONI PRINCIPALI
clusterizzazione_grafico_onoff = "ON" #fornisce rappresentazione 3D della clusterizzazione
...
def clusterizzazione(eps=distanza_max_clusterizzazione, min_samples=numero_minimo_elementi):
    ...
    if clusterizzazione_grafico_onoff == "ON":
        for counter_plot in range (0,4):
            #FIGSIZE aggiusta le proporzioni, sennò la larghezza dell'immagine è maggiore dell'altezza e l'asse z sembra
            #Più corto di x e y
            #FRAMEON toglie la grid e gli assi
            ...
            ax.set_ylabel('Y(Å)')
            ax.set_zlabel('Z(Å)')
            #Disegna i bordi della scatola
            points = np.array([[-19.803, -19.803, -25],
                               [19.803, -19.803, -25],
                               [19.803, 19.803, -25],
                               [-19.803, 19.803, -25],
                               [-19.803, -19.803, 25],
            ...
            ax.add_collection3d(Poly3DCollection(verts,facecolors='cyan', linewidths=0.5, edgecolors='black', alpha=.001))
            #Disegna NaCl
            points = np.array([[-19.803, -19.803, -5.658],
                               [19.803, -19.803, -5.658],
                               [19.803, 19.803, -5.658],
                               [-19.803, 19.803, -5.658],
                               [-19.803, -19.803, 5.658],
            ...
            verts = [[points[0], points[1], points[2], points[3]],
                     ...
                     [points[4], points[7], points[3], points[0]]]
            ax.add_collection3d(
                Poly3DCollection(verts, facecolors='grey', linewidths=0.5, edgecolors='grey', alpha=.1,zorder=1))
            #Scatter degli ossigeni
            ax.scatter(data_solo_ossigeno[:, 0], data_solo_ossigeno[:, 1], data_solo_ossigeno[:, 2], c=db_labels, s=150,depthshade=False,edgecolors='black', linewidths=0.5,alpha=0.7,antialiased=True,zorder=10)
            #Angolo di visione del plot3d, per capire che valori scegliere basta guardare che valori assumono in un plot muovendolo
            if counter_plot == 0:
                ax.view_init(elev=10,azim=-139)
                """BLOCK serve per fermare il plotting e poter vedere l'immagine rotabile"""
                #plt.show(block=True)
...
# CARICAMENTO FILES
    ...
    for x in range(len(files_pdb)):
        ...
        if funzione_della_distanza_onoff == "ON":
            ...
            """SENZARUMORE, sotto qui la versione con rumore"""
            #legend = plt.legend(['Strati', 'Isole', 'Rumore'], loc='upper right', scatterpoints=1)
            legend = plt.legend(['Strati', 'Isole'], loc='upper right', scatterpoints=1)
            ...
            plt.ylabel(r'$S_\overline{x}$',wrap=True, fontsize=11, style='oblique')
        ...
        if funzione_della_distanza_onoff == "ON" and calcolo_angoli_onoff == "ON":
            ...
            for distanza_istogrammi in np.arange(6.5, 25.5, 0.5):
                ...
                if unità_di_misura_angoli == "COSENO":
                    ...
                    plt.xticks(np.arange(-1, 1.2, 0.2), rotation=30, fontsize=16)
                    plt.xlabel('cos(θ)', fontsize=18)
